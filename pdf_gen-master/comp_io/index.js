...
async function retrieveGraphData() {
  ...
  const { numberOfDataPoints, dataPoints } = await loadJson('./compData.json');
  ...
  const questions = [
    {
      type: 'input',
      name: 'value',
      message: 'What is the next value? (0-100)%: ',
      validate(value) {
        if (parseFloat(value.trim()) > 0 && parseFloat(value.trim()) <= 100) {
          return true;
        }
...
async function addNewDataPoint(newDataPoint) {
  ...
  const { dataPoints: currentDataPoints } = await loadJson('./compData.json');

  currentDataPoints.push(refinedDataPoint);
  const nextSet = currentDataPoints.slice(-12);
  await writeJson('./compData.json', { numberOfDataPoints: currentDataPoints.length, dataPoints: nextSet });
